    ...
    <section class="pricing_wrapper">
        <div class="price_text_area">
            <div class="container" style="text-align: center;">
                ...
                <div
                    style="position: absolute; right: 10px; top: 15px; background: white; border-radius: 50%; width: 35px; height: 35px;">
                            ...
                            <div class="pricing_area">
                                @foreach ($plan->timePlans as $timePlan)
                                    <div class="form-check">
                                        <input class="form-check-input" wire:model.live='plan_id' type="radio"
                                            value="{{ $timePlan->id }}" name="flexRadioDefault"
                                            id="basicCheckBox_{{ $timePlan->id }}" />
                                        <label class="form-check-label" for="basicCheckBox_{{ $timePlan->id }}">
                                            <h5 class="title" style="font-size: 16px;"><b>{{ $timePlan->name }}</b> -
                                                <small>€{{ $timePlan->price }}/month</small></h5>
                                            <p class="sub_title" style="font-size: 12px;">{{ $timePlan->description }}
                                            </p>
                                        </label>
                                    </div>
                                @endforeach
